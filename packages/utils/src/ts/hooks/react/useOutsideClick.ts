...
export function useOutsideClick<T extends HTMLElement>(ref: RefObject<T>, config: OutsideClickConfig = NIL) {
  useEffect(() => (ref.current ? initOutsideClick(ref.current, config) : undefined), [ref, config.enabled, config.onOutsideClick, config.outOnEscape, config.outOnClick, config.outOnFocusOut, config.allowInputs, config.root, config.scoped, config.capture]);
}
