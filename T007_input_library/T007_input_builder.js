...
dropZone.addEventListener( "drop", async e => {
    ...
    let placeholder = draggedType === 'textarea' || draggedType === 'text' || draggedType === 'email' || draggedType === 'password' ? await Prompt( "Enter placeholder (optional):", "" ) : "";
...
window.editField = async function editField(id) {
  ...
  // Update options if it's a group field
...
window.saveToLocal = function saveToLocal (e) {
    ...
    if (document.activeElement.tagName == 'BUTTON') Toast({ data: { type: "success", body: "Saved Session to Local Storage" } })
}
...
window.loadFromLocal = function loadFromLocal () {
    ...
    Toast({ data: { type: "success", body: "Loaded session from Local Storage" } })
}

window.submitForm = function submitForm() {
    Toast({ data: { type: "info", body: "Simulating Form Submission" } })
}
